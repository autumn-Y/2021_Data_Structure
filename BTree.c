...
typedef struct BTNode {
    ...
    //3-WAY B-Tree이기 때문에 최대 2개의 키값을 갖지만
...
BTNode* BTInsert(BTNode *root, int key) {
    ...
    //해당 노드가 leaf 노드가 아닐 때
        ...
        //insertion 수행 후 child 노드로 들어가 leaf 노드까지 탐색
...
BTNode* splitChild(BTNode* root) {
    ...
    //이때 child node pointer는 키 값보다 1개 많으므로
    //for문이 끝난 후 한 번 더 수행
    newSibling->childs[i-(mid+1)]=root->childs[i];
    root->childs[i]=NULL;
...
void inorderTraveral(BTNode *root) {
    int i;
    printf("\n");
    for(i=0;i<root->n;i++) {
        //leaf 노드가 아니라면 밑으로 탐색
        if(!(root->isLeaf)) {
            inorderTraveral(root->childs[i]);
            printf(" ");
        }

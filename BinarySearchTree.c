...
void deleteTreeNode(Node** p, int value)
{
	...
	Node* succNode;   //Degree 2 노드를 지울 때 필요한 succesor 노드
	...
	{	
		...
		Node* psuccNode=searchTreeParentNode(*p,succNode->data);
		if(psuccNode->rightChild==succNode) {
			psuccNode->rightChild=NULL;
		}
		else {
			psuccNode->leftChild=succNode->rightChild;
		}
